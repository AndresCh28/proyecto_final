          ...
          <form className="stack-form" onSubmit={canEdit ? handleSaveBudget : handleCreateBudgetRequest}>
            <label>
              ...
              <input
                ...
                min="0"
                step="0.01"
                value={canEdit ? budgetForm.montoEstimado : solicitudForm.monto}
                onChange={(event) =>
                  canEdit
                    ? setBudgetForm((current) => ({ ...current, montoEstimado: event.target.value }))
                    : setSolicitudForm((current) => ({ ...current, monto: event.target.value }))
                }
              />
            </label>
            ...
            {movimientos.map((item) => (
              <article className="movement-card" key={item.id_movimiento}>
                <div className="movement-card-top">
                  <span className={`status-badge ${item.tipo === 'gasto' ? 'danger-badge' : 'success-badge'}`}>
                    {item.tipo}
                  </span>
