...
export function ComisionesPage() {
  ...
  const filteredComisiones = useMemo(() => {
    const normalized = listSearch.trim().toLocaleLowerCase()
    return comisiones.filter((item) => {
      const matchesText = !normalized || `${item.titulo} ${item.descripcion ?? ''}`.toLocaleLowerCase().includes(normalized)
      const matchesState = listState === 'todos' || String(item.id_estado) === listState
      return matchesText && matchesState
    })
  }, [comisiones, listSearch, listState])
  const paginatedComisiones = filteredComisiones.slice((listPage - 1) * listPageSize, listPage * listPageSize)
      ...
      <div className="panel-grid commissions-layout">
        ...
        <section className="glass-card">
          ...
          <div className="filter-bar">
            <label><span>Buscar</span><input value={listSearch} placeholder="Título o descripción" onChange={(event) => { setListSearch(event.target.value); setListPage(1) }} /></label>
            <label><span>Estado</span><select value={listState} onChange={(event) => { setListState(event.target.value); setListPage(1) }}><option value="todos">Todos los estados</option>{estados.map((estado) => <option key={estado.id_estado} value={estado.id_estado}>{estado.nombre}</option>)}</select></label>
            <button type="button" className="ghost-button" onClick={() => { setListSearch(''); setListState('todos'); setListPage(1) }}>Limpiar</button>
          </div>

          <div className="commission-list">
            {loading ? <LoadingState label="Cargando comisiones…" /> : null}

            {!loading && filteredComisiones.length === 0 ? (
          ...
          <Pagination page={listPage} pageSize={listPageSize} total={filteredComisiones.length} onPageChange={setListPage} />
        </section>
      </div>
